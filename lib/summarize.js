...
// Classify an error message into a limit type used by the UI to show targeted hints
export function classifyLimitType(rawMsg) {
  const msg = String(rawMsg || '');
  if (/supadata/i.test(msg) && /limit|quota|credits/i.test(msg)) return 'supadata';
  if (/(?:openai|custom ai)/i.test(msg) && /limit|quota/i.test(msg)) return 'openai';
  if (/youtube/i.test(msg) && /limit|quota/i.test(msg)) return 'youtube';
  if (/limit[ -]?exceeded|quota|rate[ -]?limit/i.test(msg)) return 'rate_limit';
  return null;
}
...
Start with a section titled "**At a glance**" containing 3-5 sentences capturing the essence.
