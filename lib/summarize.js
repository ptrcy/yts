...
export async function fetchTranscript(videoId, transcriptApiKey) {
  ...
  try {
    ...
    const bothRateLimited = ytErr?.message?.includes('429') && err?.message?.includes('429');
    if (bothRateLimited) {
      throw new Error('Transcript unavailable: both providers are rate-limiting this server\'s IP. Try again later.');
    }
    throw err;
  }
}
